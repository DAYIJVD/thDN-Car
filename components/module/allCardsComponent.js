...
const AllCards = () => {
    ...
    return (<>
        ...
        <div className="mx-[2rem] my-[4rem] flex flex-wrap justify-center">
            {data.map(card=> <Card key={card.id} id={card.id} name={card.name} model={card.model} location={card.location} price={card.price} year={card.year} image={card.image} category={card.category} distance={card.distance}/>)}  
        </div>
        </>
    );
}
